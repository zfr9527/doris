...
Status SegmentIterator::_apply_bitmap_index_except_leafnode_of_andnode(
    ...
    RETURN_IF_ERROR(pred->evaluate(_bitmap_index_iterators[pred->column_id()].get(), _segment->num_rows(),
                                   output_result));
    return Status::OK();
}
...
Status SegmentIterator::_apply_inverted_index_except_leafnode_of_andnode(
    ...
    RETURN_IF_ERROR(pred->evaluate(*_schema, _inverted_index_iterators[pred->column_id()].get(), num_rows(),
                                   output_result));
    return Status::OK();
}
