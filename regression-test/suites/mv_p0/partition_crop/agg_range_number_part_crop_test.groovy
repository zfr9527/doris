// Licensed to the Apache Software Foundation (ASF) under one
// or more contributor license agreements.  See the NOTICE file
// distributed with this work for additional information
// regarding copyright ownership.  The ASF licenses this file
// to you under the Apache License, Version 2.0 (the
// "License"); you may not use this file except in compliance
// with the License.  You may obtain a copy of the License at
//
//   http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing,
// software distributed under the License is distributed on an
// "AS IS" BASIS, WITHOUT WARRANTIES OR CONDITIONS OF ANY
// KIND, either express or implied.  See the License for the
// specific language governing permissions and limitations
// under the License.

suite("agg_range_number_part_crop", "mv_part_crop") {
    String db = context.config.getDbNameByFile(context.file)
    String orders_tb = "agg_orders_range_number_crop_part"

    sql """
    drop table if exists ${orders_tb}
    """

    sql """CREATE TABLE `${orders_tb}` (
      `o_orderkey` BIGINT NULL,
      `o_custkey` INT not NULL,
      `o_orderdate` DATE not null,
      `o_orderstatus` VARCHAR(1) replace,
      `o_totalprice` DECIMAL(15, 2) sum,
      `o_orderpriority` VARCHAR(15) replace,
      `o_clerk` VARCHAR(15) replace,
      `o_shippriority` INT sum,
      `o_comment` VARCHAR(79) replace
    ) ENGINE=OLAP
    aggregate KEY(`o_orderkey`, `o_custkey`, `o_orderdate`)
    COMMENT 'OLAP'
    partition by range (o_orderkey) (
        FROM (1) TO (5) INTERVAL 1
    )
    DISTRIBUTED BY HASH(`o_orderkey`) BUCKETS 96
    PROPERTIES (
    "replication_allocation" = "tag.location.default: 1"
    );"""

    sql """alter table ${orders_tb} add partition p_x values less than ("1")"""

    sql """
    insert into ${orders_tb} values 
    (null, 1, '2023-10-17', 'k', 99.5, 'a', 'b', 1, 'yy'), 
    (2, 1, '2023-10-17', 'k', 99.5, 'a', 'b', 1, 'yy'),
    (1, 2, '2023-10-18', 'o', 109.2, 'c','d',2, 'mm'),
    (3, 3, '2023-10-19', null, 99.5, 'a', 'b', 1, 'yy'),
    (4, 2, '2023-10-20', 'o', null, 'a', 'b', 1, 'yy');
    """
    sql """analyze table ${orders_tb} with sync;"""

    def create_mv_orders = { mv_name, mv_sql ->
        sql """DROP MATERIALIZED VIEW IF EXISTS ${mv_name} on ${orders_tb};"""
        sql """DROP TABLE IF EXISTS ${mv_name}"""
        sql"""
        CREATE MATERIALIZED VIEW ${mv_name} 
        AS  
        ${mv_sql}
        """
    }

    def mv_name = "agg_rang_number_crop_part_mtmv"
    def mv_sql = """
        select o_orderkey, o_custkey, o_orderdate
        from ${orders_tb} 
        group by o_orderkey, o_custkey, o_orderdate
        """
    create_mv_orders(mv_name, mv_sql)
    waitingMVTaskFinished(orders_tb, mv_name)

    def query_sql = """
        select o_orderkey, o_custkey, o_orderdate
        from ${orders_tb} 
        where o_orderkey = 1
        group by o_orderkey, o_custkey, o_orderdate
        """
    explain {
        sql("${query_sql}")
        contains "partitions=1/5 (p_1_2)"
    }
    explain {
        sql("${query_sql}")
        contains "(${mv_name})"
    }

    query_sql = """
        select o_orderkey, o_custkey, o_orderdate
        from ${orders_tb} 
        where o_orderkey >= 1 and o_orderkey < 2
        group by o_orderkey, o_custkey, o_orderdate
        """
    explain {
        sql("${query_sql}")
        contains "partitions=1/5 (p_1_2)"
    }
    explain {
        sql("${query_sql}")
        contains "(${mv_name})"
    }

    query_sql = """
        select o_orderkey, o_custkey, o_orderdate
        from ${orders_tb} 
        where o_orderkey <> 1
        group by o_orderkey, o_custkey, o_orderdate
        """
    explain {
        sql("${query_sql}")
        contains "partitions=4/5 (p_2_3,p_3_4,p_4_5,p_x)"
    }
    explain {
        sql("${query_sql}")
        contains "(${mv_name})"
    }

}
