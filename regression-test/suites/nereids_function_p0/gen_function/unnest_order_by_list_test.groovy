...
suite("unnest_order_by_list_test", "unnest") {
    ...
    sql """
        ...
        (1, 'Alice', ARRAY[90, 70, 85], bitmap_from_string("10, 20")),
        (2, 'Bob', ARRAY[60, 80], bitmap_from_string("5, 15")),
        (3, 'Charlie', ARRAY[95], bitmap_from_string("30"));"""
    ...
    qt_order_by_ordinality_and_value """SELECT user_name, s.val, s.pos FROM ${tb_name1}, UNNEST(scores) WITH ORDINALITY AS s(val, pos) ORDER BY s.pos ASC, s.val DESC;"""
    ...
    qt_order_by_column_and_ordinality """
        ...
        FROM ${tb_name1}, UNNEST(scores) WITH ORDINALITY AS s(val, pos)
    ...
    qt_order_by_unnest_alias_in_select """SELECT user_name, s.score_val FROM ${tb_name1}, UNNEST(scores) AS s(score_val) ORDER BY s.score_val DESC;"""

    // Test ordering by the UNNEST function call directly in the ORDER BY clause.
    qt_order_by_unnest_function_call """SELECT user_name FROM ${tb_name1}, UNNEST(scores) as s(score) ORDER BY s.score;"""
    ...
    sql """
        ...
        (1, 'Math', ARRAY[80, 95, 70]),
        (1, 'English', ARRAY[90, 85]),
        (2, 'Math', ARRAY[60, 75]);"""
    ...
    qt_order_by_unnested_from_left_join """
        SELECT 
            ...
            t.tag 
        FROM ${tb_name1} u 
        LEFT JOIN UNNEST(u.scores) AS t(score) ON true
