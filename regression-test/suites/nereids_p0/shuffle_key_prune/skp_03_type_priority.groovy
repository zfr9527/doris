// ===========================================================================================
// 分类 03：类型优先级选择 — D01-D09, H01-H07
// ===========================================================================================
//
// 【测试目的】
//   验证 Shuffle Key Pruning 的类型优先级排序逻辑。当多列都满足 NDV 和 balanced 条件时，
//   Optimizer 按数据类型的 hash 计算效率选择最优列。
//
// 【类型优先级】（从高到低）
//   bigint/largeint > decimal > datev2/datetimev2 > char > varchar > string
//   如果最高优先级有 skew 则跳过，选下一个。
//
// 【D 组 Case 列表】
//   D01: 可选 largeint vs 有skew列 → 选 li_good（正例）
//   D02: 可选 decimal vs 有skew列 → 选 de_good（正例）
//   D03: 可选 datev2 vs 有skew列 → 选 d_good（正例）
//   D04: 可选 datetimev2 vs 有skew列 → 选 dt_good（正例）
//   D05: 可选 char(8) vs 有skew列 → 选 ch8_good（正例）
//   D06: ch8_low 有 skew → 跳过 char, 选 vc16_good（正例）
//   D07: ch8_low + vc16_hot 都 skew → 选 s_good（string 兜底）
//   D08: s_hot 有 skew → 跳过 string, 选 bi_good（正例）
//   D09: 分布已满足 (distributed by hash(li_good)) → 不改（负例）
//
// 【H 组 Case 列表】（基础类型 Smoke Test）
//   H01: int/bigint/decimal/string → 选 bi_key（bigint 优先）
//   H02: bigint + hot_num(skew) → 跳过 hot_num, 选 bi_key
//   H03: bigint + null_num(null占比高) → 跳过 null_num, 选 bi_key
//   H04: 8 列混合类型 → 仍选 bi_key（bigint 优先）
//   H05: 人为降低 bigint NDV, 提高 string NDV → 选 s_key（string NDV 占优）
//   H06: hot_str 有 skew → 跳过, 选 bi_key
//   H07: hash(bi_key) 表 → 分布已满足 → 不改（负例）
//
// 【验证方法】
//   通过注入不同列的 NDV、hot_values 控制 optimizer 的选择，验证 Plan 变化和选中列。
// ===========================================================================================
suite("skp_03_type_priority", "agg_shuffle_prune_func") {
