// ===========================================================================================
// 分类 04：回退与保守逻辑 — R01-R02, SK01, N01-N02, E01-E02
// ===========================================================================================
//
// 【测试目的】
//   验证 Shuffle Key Pruning 在特殊场景下的回退（不优化）行为：
//   1. ROLLUP/GROUPING SETS → Repeat 算子会产生多组不同的 group by key → 不 prune
//   2. 所有候选列都有 skew → 无安全列可选 → 不 prune
//   3. 统计信息缺失/部分缺失 → 保守回退 → 不 prune
//   4. String 回退：Step 1 选不到单列时，进入 Step 2 删除 string 类型列
//
// 【Case 列表】
//   R01: GROUP BY ROLLUP(b,c,d,e,f) → Repeat 算子 → 不 prune（负例）
//   R02: GROUP BY GROUPING SETS → 同理 → 不 prune（负例）
//   SK01: k1~k5 全部 hot_values=0.8 → 全 skew → 不 prune（负例）
//   N01: 分区表部分分区无统计 → 保守 → 不 prune（负例）
//   N02: 只分析了 b 列 → 其他列缺 stats → 不 prune（负例）
//   E01: 所有 non-string 列都 skew/低NDV → Step 2 移除 string 后剩 3 列联合 key（正例）
//   E02: 只有 string 列 → 删完 string 后无列可用 → 不 prune（负例）
//
// 【验证方法】
//   检查 Plan 是否变化，并验证结果一致性。
// ===========================================================================================
suite("skp_04_degrade_fallback", "agg_shuffle_prune_func") {
    ...
    // R 组是负例，analyze 的低 row_count 不影响断言
