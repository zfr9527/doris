// ===========================================================================================
// 分类 05：Expression Key 与 Bucket 变化 — EX01-EX04, BK01-BK02
// ===========================================================================================
//
// 【测试目的】
//   验证以下两类场景：
//   A. Expression Key：GROUP BY 中包含表达式而非纯列引用时的 prune 行为
//   B. Bucket 数量变化：不同 bucket 数对 prune 逻辑的影响
//
// 【EX 组 Case 列表】
//   EX01: cast(b as bigint) → 类型转换折回原 slot → prune 正常生效（正例）
//   EX02: substr(s_key,1,8) → 字符串截取产生 expression slot → 可被选中（正例）
//         其中 b,c,d,e 注入 80% skew 确保它们被排除
//   EX03: coalesce(null_k,-1) → unknown stats → 不确定性 → 不 prune（负例）
//   EX04: date_trunc(dt_key,'day') → 不稳定分布 → 不 prune（负例）
//
// 【BK 组 Case 列表】
//   BK01: 1 bucket 表 → prune 仍生效（正例）
//   BK02: 32 bucket 表 → prune 仍生效（正例）
//   说明：bucket 数量不影响 prune 逻辑（prune 作用于 shuffle exchange，不依赖 bucket）
//
// 【验证方法】
//   检查 Plan 的 orderedShuffledColumns 变化，验证结果一致性。
// ===========================================================================================
suite("skp_05_expr_bucket", "agg_shuffle_prune_func") {
    ...
    // EX01: cast(b as bigint) → 折回 slot → prune 生效
    ...
    // EX02: substr → expression slot → 可被选中
    applyExpr(guard)
    ...
    // EX03: coalesce → unknown stats → 不 prune
    applyExpr(guard)
