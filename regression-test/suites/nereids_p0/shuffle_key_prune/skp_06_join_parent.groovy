// ===========================================================================================
// 分类 06：Join 路径 + Parent Request — J01-J12, P01-P02
// ===========================================================================================
//
// 【测试目的】
//   验证 Shuffle Key Pruning 与 Join 算子的交互行为，以及 Parent Request（父节点
//   请求子节点调整 shuffle key 以消除额外 exchange）的正确性。
//
// 【核心背景】
//   - 当 Agg 的输出被 Join 消费时，Agg 的 shuffle key 可以被调整为 Join key 的子集，
//     从而省去 Join 之前的 re-shuffle exchange。
//   - tByB = distributed by hash(b) → 如果 group by 包含 b 则可能已满足分布 → 不 prune
//   - tByX = distributed by hash(x) → x 不在 group by 中 → 需要 shuffle → 可以 prune
//
// 【J 组 Case 列表】
//   J01: scan JOIN agg(tByB) → 分布已满足 → 不 prune（负例）
//   J02: scan JOIN agg(tByX) → 需要 shuffle → prune 生效（正例）
//   J03: agg(tByB) JOIN agg(tByB) → 双侧均满足 → 不 prune（负例）
//   J04: agg(tByX) JOIN agg(tByX) → 双侧需 shuffle → prune 生效（正例）
//   J05: LEFT JOIN + agg(tByX) → 同 J02 → prune 生效（正例）
//   J06: LEFT SEMI JOIN → prune 生效（正例）
//   J07: LEFT ANTI JOIN → prune 生效（正例）
//   J08: agg(tByB) JOIN scan on 非 group-by 列 → 不满足 → 不 prune（负例）
//   J09: scan JOIN scan → 无 agg → 不 prune（负例）
//   J10: agg on join output + 全 skew 列 → 无安全列 → 不 prune（负例）
//   J11: parent key subset 正例 → agg shuffle key 被调整为 join key 子集
//   J12: parent key subset 负例 → 低 NDV 列不满足条件 → 不调整
//
// 【P 组 Case 列表】
//   P01: subquery agg → parent agg → 内层已满足 → 不 prune（负例）
//   P02: subquery agg → window(partition by) → 内层已满足 → 不 prune（负例）
//
// 【验证方法】
//   使用 run() 检查 orderedShuffledColumns 变化；
//   使用 runParent() 检查 agg_shuffle_use_parent_key 对 shuffle key 的调整。
// ===========================================================================================
suite("skp_06_join_parent", "agg_shuffle_prune_func") {
    ...
    def runParent = { String id,String q,boolean chg,List<String> offCols,List<String> onCols ->
        ...
        sql "set enable_agg_shuffle_key_prune=false;"; sql "set disable_join_reorder=true;"; sql "set runtime_filter_mode=OFF;"
    ...
    // J10: root agg on join output, 全列临时注入 hot_values → 阻断 agg 自身 prune → 负例
    ...
    scanStats(tScan,qn,qn,qn,qn,qn) // 恢复

    // J11: parent key subset 正例
