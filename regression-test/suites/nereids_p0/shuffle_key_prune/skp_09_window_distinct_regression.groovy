// ===========================================================================================
// 分类 09：Window / Distinct Regression — WD01-WD04
// ===========================================================================================
//
// 【测试目的】
//   针对 window / distinct 形态补一组更聚焦的功能回归：
//   - Window guard：确保窗口包装下不会错误发生 prune
//   - Distinct + Window：确保 DISTINCT 聚合在窗口包装后仍保持正确的 prune / no-prune 行为
//
// 【Case 列表】
//   WD01: window over inner agg，child 分布已满足 → 不 prune（负例）
//   WD02: count(distinct) + window，高 NDV → prune 生效（正例）
//   WD03: count(distinct) + avg + window，高 NDV → prune 仍生效（正例）
//   WD04: count(distinct) + window，低 NDV → 不 prune（负例）
// ===========================================================================================
suite("skp_09_window_distinct_regression", "agg_shuffle_prune_func") {
    ...
    // WD01: window over inner agg，child 已满足，不 prune
    runCase("WD01", """
        select t.a, t.sum_v,
    ...
    // WD02: count(distinct) + window，高 NDV → prune
    ...
    // WD03: count(distinct) + avg + window，高 NDV → prune
    setupDistinctStats(highNdv)
    runCase("WD03", """
    ...
    // WD04: count(distinct) + window，低 NDV → 不 prune
