...
JOIN test_rollup_rewrite t2 ON t1.a = t2.a
WHERE t1.total > 10;"""
...
SELECT * FROM cte1 JOIN test_rollup_rewrite t2 ON cte1.a = t2.a WHERE cte1.sum_f > 0;"""
...
WITH complex_source AS (
    ...
    FROM test_rollup_rewrite t1
    JOIN test_rollup_rewrite t2 ON t1.a = t2.b
    WHERE t1.f > 0
)
SELECT a, b, c, d, e, SUM(f)
FROM complex_source
...
SELECT * FROM 
    (SELECT a, b, c, d, e, SUM(f) FROM test_rollup_rewrite GROUP BY ROLLUP(a, b, c, d, e)) temp1,
    (SELECT a, b, c, d, e, MIN(f) FROM test_rollup_rewrite GROUP BY CUBE(a, b, c, d, e)) temp2
...
WHERE t.gid < 16; -- 过滤掉某些层级的聚合结果"""
    ...
    sql """SELECT a, b, c, d, e, GROUPING(a), GROUPING_ID(a,b,c,d,e), SUM(f) FROM test_rollup_rewrite GROUP BY ROLLUP(a,b,c,d,e);"""
    sql """SELECT a, b, c, d, GROUPING(a), GROUPING(d), GROUPING_ID(a,b,c,d), SUM(f) FROM test_rollup_rewrite GROUP BY CUBE(a,b,c,d);"""
    sql """SELECT a, b, c, d, e, GROUPING_ID(a,e), SUM(f) FROM test_rollup_rewrite GROUP BY GROUPING SETS ((a,b,c,d,e), (a,b), ());"""
...
FROM test_rollup_rewrite WHERE 1=0
GROUP BY ROLLUP(a,b,c,d,e);"""
