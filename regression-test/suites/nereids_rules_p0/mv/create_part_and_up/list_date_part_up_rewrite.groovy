...
suite("mtmv_list_date_part_up_rewrite", "zfr_mtmv_test") {
    ...
    sql """
    drop table if exists orders_list_date
    """

    sql """CREATE TABLE `orders_list_date` (
    ...
    sql """
    insert into orders_list_date values 
    ...
    sql """CREATE MATERIALIZED VIEW mv1 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_list_date as t1 left join orders_list_date as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey;"""
    ...
    sql """CREATE MATERIALIZED VIEW mv2 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_list_date as t1 left join orders_list_date as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey;"""

    def sql1 = """select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_list_date as t1 left join orders_list_date as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey"""
    def sql2 = """select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_list_date as t1 left join orders_list_date as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey"""
    ...
    sql """ALTER TABLE lineitem_list_date DROP PARTITION IF EXISTS p4 FORCE"""
    for (int i = 0; i < mv_name_list.size(); i++) {
        explain {
            sql("${query_stmt_list[i]}")
            contains "${mv_name_list[i]}(${mv_name_list[i]})"
