...
suite("mtmv_list_date_part_up_rewrite", "zfr_mtmv_test") {
    ...
    String mv_prefix = "list_date_up_union"

    sql """
    drop table if exists lineitem_list_date_union
    """

    sql """CREATE TABLE `lineitem_list_date_union` (
    ...
    partition by list (`l_shipdate`) (
        partition p1 values in ("2023-10-29"), 
        partition p2 values in ("2023-10-30"), 
        partition p3 values in ("2023-10-31"))
    ...
    sql """
    drop table if exists orders_list_date_union
    """

    sql """CREATE TABLE `orders_list_date_union` (
    ...
    partition by list (`o_orderdate`) (
        ...
        partition p2 values in ("2023-10-30"), 
        partition p3 values in ("2023-10-31"),
        partition p4 values in ("2023-11-01"), 
        partition p5 values in ("2023-11-02"))
    ...
    sql """
    insert into lineitem_list_date_union values 
    ...
    sql """
    insert into orders_list_date_union values 
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv1 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_list_date_union as t1 left join orders_list_date_union as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey;"""
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv2 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_list_date_union as t1 left join orders_list_date_union as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey;"""

    def sql1 = """select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_list_date_union as t1 left join orders_list_date_union as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey"""
    def sql2 = """select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_list_date_union as t1 left join orders_list_date_union as t2 on t1.l_orderkey = t2.o_orderkey group by col1, l_shipdate, l_orderkey"""
    ...
    sql """alter table lineitem_list_date_union add partition p4 values in ("2023-11-01");"""
    sql """insert into lineitem_list_date_union values 
    ...
    sql """ALTER TABLE lineitem_list_date_union DROP PARTITION IF EXISTS p4 FORCE"""
    for (int i = 0; i < mv_name_list.size(); i++) {
        explain {
            sql("${query_stmt_list[i]}")
            contains "${mv_name_list[i]}(${mv_name_list[i]})"
