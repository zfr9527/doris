...
suite("mtmv_range_date_datetrunc_date_part_up_multi_pct_tables") {
    ...
    for (int i = 0; i < select_lists.size(); i++) {
        for (int j = 0; j < select_lists.size(); j++) {
            if (i == j || j > 5) {
                ...
                def group_by_str = " L_LINENUMBER, O_CUSTKEY, " + select_lists[j].replaceAll("as col1", "")
                def str = "select L_LINENUMBER, O_CUSTKEY, " + select_lists[i] + " from ${tb_name1} inner join ${tb_name2} on l_shipdate = o_orderdate group by " + group_by_str
                sql str
