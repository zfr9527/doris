...
suite("mtmv_range_date_part_up_rewrite_multi_pct_table") {
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv1 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_range_date_union as t1 inner join orders_range_date_union as t2 on t1.l_shipdate = t2.o_orderdate group by col1, l_shipdate, l_orderkey;"""
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv2 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_range_date_union as t1 inner join orders_range_date_union as t2 on t1.l_shipdate = t2.o_orderdate group by col1, l_shipdate, l_orderkey;"""

    def sql1 = """select date_trunc(`l_shipdate`, 'day') as col1, l_shipdate, l_orderkey from lineitem_range_date_union as t1 inner join orders_range_date_union as t2 on t1.l_shipdate = t2.o_orderdate group by col1, l_shipdate, l_orderkey"""
    def sql2 = """select date_trunc(`l_shipdate`, 'hour') as col1, l_shipdate, l_orderkey from lineitem_range_date_union as t1 inner join orders_range_date_union as t2 on t1.l_shipdate = t2.o_orderdate group by col1, l_shipdate, l_orderkey"""
    ...
    for (int i = 0; i < mv_name_list.size(); i++) {
        ...
        def mv_infos = sql "select RefreshMode, NeedRefreshPartitions, Progress from tasks('type'='mv') where MvName='${mv_name_list[i]}' order by CreateTime desc limit 1"
        logger.info("mv_infos1:" + i + ", " + mv_infos)
    }
    ...
    for (int i = 0; i < mv_name_list.size(); i++) {
        ...
        def mv_infos = sql "select RefreshMode, NeedRefreshPartitions, Progress from tasks('type'='mv') where MvName='${mv_name_list[i]}' order by CreateTime desc limit 1"
        logger.info("mv_infos2:" + i + ", " + mv_infos)
    }
    ...
    for (int i = 0; i < mv_name_list.size(); i++) {
        waitingMTMVTaskFinishedByMvName(mv_name_list[i])
    }

    sql """alter table orders_range_date_union add partition p6 values [("2023-11-03"), ("2023-11-04"));"""
    sql """insert into orders_range_date_union values (4, 5, 'k', 99.5, 'a', 'b', 1, 'yy', '2023-11-03');"""
    for (int i = 0; i < mv_name_list.size(); i++) {
        mv_rewrite_any_success(query_stmt_list[i], mv_name_list)
        compare_res(query_stmt_list[i] + " order by 1,2,3")
    }
    ...
    sql """ALTER TABLE orders_range_date_union DROP PARTITION IF EXISTS p6 FORCE"""
    for (int i = 0; i < mv_name_list.size(); i++) {
        mv_rewrite_any_success(query_stmt_list[i], mv_name_list)
        compare_res(query_stmt_list[i] + " order by 1,2,3")
    }
