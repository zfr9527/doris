...
suite("mtmv_range_date_part_up_union_rewrite_multi_pct_table") {
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv1 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'day') as col1, l_orderkey from lineitem_range_date_union as t1 union all select o_orderdate, o_orderkey from orders_range_date_union as t2;"""
    ...
    sql """CREATE MATERIALIZED VIEW ${mv_prefix}_mv2 BUILD IMMEDIATE REFRESH AUTO ON MANUAL partition by(date_trunc(`col1`, 'month')) DISTRIBUTED BY RANDOM BUCKETS 2 PROPERTIES ('replication_num' = '1') AS  
        select date_trunc(`l_shipdate`, 'hour') as col1, l_orderkey from lineitem_range_date_union as t1 union all select o_orderdate, o_orderkey from orders_range_date_union as t2;"""

    def sql1 = """select date_trunc(`l_shipdate`, 'day') as col1, l_orderkey from lineitem_range_date_union as t1 union all select o_orderdate, o_orderkey from orders_range_date_union as t2 """
    def sql2 = """select date_trunc(`l_shipdate`, 'hour') as col1, l_orderkey from lineitem_range_date_union as t1 union all select o_orderdate, o_orderkey from orders_range_date_union as t2 """
