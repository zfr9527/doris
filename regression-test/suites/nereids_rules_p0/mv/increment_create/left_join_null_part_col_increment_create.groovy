...
suite("left_join_null_part_col_increment_create") {
    ...
    PARTITION p0 VALUES LESS THAN ('20231017'),
    PARTITION p1 VALUES [('20231017'), ('20231018')),
    PARTITION p2 VALUES [('20231018'), ('20231019')),
    PARTITION p3 VALUES [('20231019'), ('20231020')),
    PARTITION p4 VALUES [('20231020'), ('20231021')),
    PARTITION p5 VALUES [('20231021'), ('20231022')),
    PARTITION p6 VALUES [('20231022'), ('20231023'))
    )
    ...
    (null, 1, 'o', 99.5, 'a', 'b', 1, 'yy', 'null'),
    (1, null, 'k', 109.2, 'c','d',2, 'mm', '2023-10-17'),
    (3, 3, null, 99.5, 'a', 'b', 1, 'yy', '2023-10-19'),
    (1, 2, 'k', null, 'a', 'b', 1, 'yy', '2023-10-20'),
    (2, 3, 'o', 109.2, null,'d',2, 'mm', '2023-10-21'),
    (3, 1, 'o', 99.5, 'a', null, 1, 'yy', '2023-10-22'),
    (1, 3, 'k', 99.5, 'a', 'b', null, 'yy', '2023-10-19'),
    (2, 1, 'o', 109.2, 'c','d',2, null, '2023-10-18'),
    (null, 2, 'k', 99.5, 'a', 'b', 1, 'yy', 'null'),
    (null, 5, 'o', 99.5, 'a', 'b', 1, 'yy', 'null'); 
    """

    sql """
    insert into lineitem values 
    (null, 1, 2, 3, 5.5, 6.5, 7.5, 8.5, 'o', 'k', '2023-10-17', '2023-10-17', 'a', 'b', 'yyyyyyyyy', 'null'),
    (1, null, 3, 1, 5.5, 6.5, 7.5, 8.5, 'o', 'k', '2023-10-18', '2023-10-18', 'a', 'b', 'yyyyyyyyy', '2023-10-18'),
    ...
    (2, 3, 2, 1, 5.5, 6.5, 7.5, 8.5, 'o', 'k', null, '2023-10-18', 'a', 'b', 'yyyyyyyyy', 'null'),
    (null, 1, 1, 2, 7.5, 8.5, 9.5, 10.5, 'k', 'o', '2023-10-19', null, 'c', 'd', 'xxxxxxxxx', '2023-10-19'),
    (null, 3, 2, 2, 5.5, 6.5, 7.5, 8.5, 'o', 'k', '2023-10-17', '2023-10-17', 'a', 'b', 'yyyyyyyyy', 'null');
    """
...
//        assert (refresh_info[0][15] == "[\"p_20231017_20231018\",\"p_20231019_20231020\",\"p_20231018_20231019\"]")
//        assert (refresh_info[0][16] == "[\"p_20231017_20231018\",\"p_20231019_20231020\",\"p_20231018_20231019\"]")
    ...
    def primary_tb_null_change = {
        sql """
        insert into lineitem values 
        (2, 3, 2, 2, 5.5, 6.5, 7.5, 8.5, 'o', 'k', '2023-10-17', '2023-10-17', 'a', 'b', 'yyyyyyyyy', 'null');
        """
    }
    ...
    // group by function
    def mv_sql_10 = """select l_shipdate, l_orderkey 
        from lineitem
        left join orders
        on lineitem.l_shipdate = orders.o_orderdate
        group by l_shipdate, l_orderkey """
    ...
    for (int i = 0; i < sql_all_list.size(); i++) {
        ...
        if (sql_all_list[i] in sql_error_list) {
            try {
                create_mv_left_part(sql_all_list[i], partition_by_part_col)
            } catch (Exception e) {
                logger.info("e.message: " + e.message)
                assert(e.message.indexOf("Unable to find a suitable base table for partitioning") != -1)
            }
        } else {
            create_mv_left_part(sql_all_list[i], partition_by_part_col)
    ...
    for (int i = 0; i < sql_all_list.size(); i++) {
        ...
        if (sql_all_list[i] in sql_error_list) {
            try {
                create_mv_left_part(sql_all_list[i], partition_by_not_part_col)
            } catch (Exception e) {
                logger.info("e.message: " + e.message)
                assert(e.message.indexOf("Unable to find a suitable base table for partitioning") != -1)
            }
