...
suite("mtmv_life_lock_test") {
    ...
    def waitingChangeTaskFinished = { def curDb ->
        Thread.sleep(2000)
        sql """use ${curDb}"""
        String showTasks = "SHOW ALTER TABLE COLUMN order by CreateTime DESC limit 1;"
        String status = "NULL"
        ...
        connect(user = "root", password = "", url = context.config.jdbcUrl) {
            ...
            while (judge_mtmv_res && judge_table_res) {
                try {
                    ...
                    if (!waitingChangeTaskFinished(db)) {
                        judge_table_res = false
                    }
    ...
    while (true) {
        ...
        logger.info("table data change + mtmv create")
        init_environment()
        sql mtmv_drop1
        sql mtmv_drop2
