...
suite("full_join_filter_hive") {
    ...
    String ctl = "mv_rewrite_full_filter_hive"
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_full_join"
            def lineitem_tb = "lineitem_full_join"
            ...
            def mv_stmt_1 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey
                ...
                full join (select o_orderdate,o_orderkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate > '2023-10-17' ) t 
            ...
            def mv_stmt_4 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey  
                ...
                full join `${catalog_name}`.`${db}`.${order_tb} 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where l_shipdate > '2023-10-18'  and o_orderdate > '2023-10-18'"""

            def mv_stmt_5 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb} 
                full join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from `${catalog_name}`.`${db}`.${lineitem_tb}  where l_shipdate > '2023-10-17') t 
                on t.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""

            def mv_stmt_6 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey 
                from (select o_orderdate, o_orderkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate > '2023-10-17' ) t 
                full join `${catalog_name}`.`${db}`.${lineitem_tb}   
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = t.o_orderkey"""

            def mv_stmt_7 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb}  
                full join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where l_shipdate > '2023-10-16' """
            ...
            def mv_list_1 = [mv_stmt_0, mv_stmt_1, mv_stmt_2, mv_stmt_3, mv_stmt_4, mv_stmt_5, mv_stmt_6,
                             mv_stmt_7, mv_stmt_8, mv_stmt_9]
            for (int i = 0; i < mv_list_1.size(); i++) {
                logger.info("i:" + i)
                def mv_name = """mv_name_full_join_${i}"""
                create_all_mv(mv_name, mv_list_1[i])
