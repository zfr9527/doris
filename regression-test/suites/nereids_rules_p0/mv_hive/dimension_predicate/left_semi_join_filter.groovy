 ...
 */
suite("left_semi_join_filter_hive", "dimension_predicate") {
    ...
    String ctl = "mv_rewrite_left_semi_filter_hive"
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_left_semi_join"
            def lineitem_tb = "lineitem_left_semi_join"
            ...
            def mv_stmt_0 = """select t.l_shipdate, t.l_partkey, t.l_suppkey  
                ...
                left semi join `${catalog_name}`.`${db}`.${order_tb} 
            ...
            def mv_stmt_1 = """select l_shipdate, l_partkey, l_suppkey 
                ...
                left semi join (select o_orderdate,o_orderkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate > '2023-10-17' ) t 
            ...
            def mv_stmt_4 = """select o_orderdate, o_orderkey, o_custkey 
                ...
                left semi join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from `${catalog_name}`.`${db}`.${lineitem_tb}  where l_shipdate > '2023-10-17') t 
                on t.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""
            ...
            def mv_stmt_5 = """select t.o_orderdate, t.o_orderkey, t.o_custkey 
                ...
                left semi join `${catalog_name}`.`${db}`.${lineitem_tb}   
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = t.o_orderkey"""
            ...
            for (int i = 0; i < mv_list_1.size(); i++) {
                logger.info("i:" + i)
                def mv_name = """mv_name_left_semi_join_${i}"""
                if (i < 4) {
