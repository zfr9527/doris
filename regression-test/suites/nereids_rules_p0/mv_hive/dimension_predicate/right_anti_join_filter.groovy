 ...
 */
suite("right_anti_join_filter_hive", "dimension_predicate") {
    ...
    String ctl = "mv_rewrite_right_anti_filter_hive"
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_right_anti_join"
            def lineitem_tb = "lineitem_right_anti_join"
            ...
            def mv_stmt_0 = """select o_orderdate, o_orderkey, o_custkey  
                ...
                right anti join `${catalog_name}`.`${db}`.${order_tb} 
            ...
            def mv_stmt_1 = """select t.o_orderdate, t.o_orderkey, t.o_custkey
                ...
                right anti join (select o_orderdate,o_orderkey,o_custkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate > '2023-10-17' ) t 
            ...
            def mv_stmt_4 = """select t.l_shipdate, t.l_partkey, t.l_suppkey  
                ...
                right anti join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from `${catalog_name}`.`${db}`.${lineitem_tb}  where l_shipdate > '2023-10-17') t 
                on t.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""
            ...
            def mv_stmt_5 = """select l_shipdate, l_partkey, l_suppkey  
                ...
                right anti join `${catalog_name}`.`${db}`.${lineitem_tb}   
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = t.o_orderkey"""
