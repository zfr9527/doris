 ...
 */
suite("inner_join_infer_and_derive_hive", "join_infer_derive") {
    ...
    String ctl = "inner_infer_derive_hive"
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_inner"
            def lineitem_tb = "lineitem_inner"
            ...
            def mtmv_full_join_stmt = """
                select l_shipdate, o_orderdate, l_partkey, l_suppkey, o_orderkey
                from `${catalog_name}`.`${db}`.${lineitem_tb}
                full join `${catalog_name}`.`${db}`.${order_tb}
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""
            def mv_name_1 = ctl + "_mv_inner"
            def mtmv_stmt_list = [mtmv_left_join_stmt, mtmv_right_join_stmt, mtmv_full_join_stmt]
            def query_stmt_0 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from (select l_shipdate, l_partkey, l_suppkey, l_orderkey from `${catalog_name}`.`${db}`.${lineitem_tb} where l_shipdate = '2023-10-17') t
                inner join `${catalog_name}`.`${db}`.${order_tb} 
            ...
            def query_stmt_1 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join (select o_orderdate,o_orderkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate = '2023-10-17' ) t 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = t.o_orderkey"""
            def query_stmt_2 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join `${catalog_name}`.`${db}`.${order_tb} 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where l_shipdate = '2023-10-17'"""
            def query_stmt_3 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join `${catalog_name}`.`${db}`.${order_tb} 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where o_orderdate = '2023-10-17'"""
            def query_stmt_4 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join `${catalog_name}`.`${db}`.${order_tb} 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where o_orderkey = 1"""
            def query_stmt_5 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join `${catalog_name}`.`${db}`.${order_tb} 
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where l_suppkey = 2"""
            def query_stmt_6 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb} 
                inner join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from `${catalog_name}`.`${db}`.${lineitem_tb}  where l_shipdate = '2023-10-17') t 
                on t.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""
            def query_stmt_7 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey 
                ...
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = t.o_orderkey"""
            def query_stmt_8 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb}  
                inner join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where l_shipdate = '2023-10-17' """
            def query_stmt_9 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb} 
                inner join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                where o_orderdate = '2023-10-17'  """
            def query_stmt_10 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb}  
                inner join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey
                where o_orderkey = 1"""
            def query_stmt_11 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb}  
                inner join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey
                where l_suppkey = 2"""
            def query_stmt_12 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${order_tb}  
                inner join `${catalog_name}`.`${db}`.${lineitem_tb}  
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey
                where l_suppkey = 2 and o_orderdate = '2023-10-17'"""
            def query_stmt_13 = """select l_shipdate, o_orderdate, l_partkey, l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                inner join `${catalog_name}`.`${db}`.${order_tb}
                on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey
                where l_suppkey = 2 and o_orderdate = '2023-10-17'"""
            ...
            for (int mtmv_it = 0; mtmv_it < mtmv_stmt_list.size(); mtmv_it++) {
                ...
                create_mv(mv_name_1, mtmv_stmt_list[mtmv_it])
