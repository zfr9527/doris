...
suite("left_join_infer_and_derive_hive", "join_infer_derive") {
    ...
    String ctl = "left_infer_derive_hive"
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_left"
            def lineitem_tb = "lineitem_left"
            ...
            def query_stmt_1 = """select l_shipdate, t.o_orderdate, l_partkey, l_suppkey, t.o_orderkey
                from `${catalog_name}`.`${db}`.${lineitem_tb}  
                left join (select o_orderdate,o_orderkey from `${catalog_name}`.`${db}`.${order_tb} where o_orderdate = '2023-10-17' ) t 
            ...
            def query_stmt_6 = """select t.l_shipdate, o_orderdate, t.l_partkey, t.l_suppkey, `${catalog_name}`.`${db}`.${order_tb}.o_orderkey 
                ...
                left join  (select l_shipdate, l_orderkey, l_partkey, l_suppkey  from `${catalog_name}`.`${db}`.${lineitem_tb}  where l_shipdate = '2023-10-17') t 
                on t.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey"""
