...
suite("nested_mtmv_hive") {
    ...
    for (String hivePrefix : ["hive2", "hive3"]) {
        try {
            ...
            def order_tb = "orders_1"
            def lineitem_tb = "lineitem_1"
            def partsupp_tb = "partsupp_1"
            ...
            def mv_stmt_4 = """
                ...
                inner join `${catalog_name}`.`${db}`.${order_tb} on `${catalog_name}`.`${db}`.${lineitem_tb}.l_orderkey = `${catalog_name}`.`${db}`.${order_tb}.o_orderkey
                inner join `${catalog_name}`.`${db}`.${partsupp_tb} on `${catalog_name}`.`${db}`.${lineitem_tb}.l_partkey = `${catalog_name}`.`${db}`.${partsupp_tb}.ps_partkey and `${catalog_name}`.`${db}`.${lineitem_tb}.l_suppkey = `${catalog_name}`.`${db}`.${partsupp_tb}.ps_suppkey
                where `${catalog_name}`.`${db}`.${lineitem_tb}.l_shipdate >= "2023-10-17" 
            ...
            def join_mv_2 = """
                ...
                from ${mv_1} as t
                inner join `${catalog_name}`.`${db}`.${partsupp_tb}
                on t.l_partkey = `${catalog_name}`.`${db}`.${partsupp_tb}.ps_partkey and t.l_suppkey = `${catalog_name}`.`${db}`.${partsupp_tb}.ps_suppkey
                where `${catalog_name}`.`${db}`.${partsupp_tb}.ps_suppkey > 1
